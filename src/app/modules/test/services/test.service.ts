...
export class TestService {
  ...
  private getConfig(options?: { referrer?: string }) {
    ...
    // The rmbtws library reports its own version as client_software_version and
    // the RMBT server version as client_version, so both are left untouched here.
  ...
  private startGeoTracking() {
    this.geoTrackerService.startGeoTracking(
      ...
      (data) => {
        this.testStore.locationPermissionDenied.set(false)
