...
export class MeasurementResult implements IMeasurementResult {
  ...
  static getPings(threadResults: IMeasurementThreadResult[]) {
    ...
    return (
      threadResults
        .reduce(
          (acc, result) => (result?.pings?.length > 0 ? result.pings : acc),
          [] as IPing[]
        )
  ...
  static getThreadSpeedItems(
    ...
    const tr = direction == "down" ? threadResult.down : threadResult.up
    if (!tr) {
      return map
    }
    for (let i = 0; i < tr.nsec.length; i++) {
      const thisNsec = Number(tr.nsec[i])
      const thisBytes = Number(tr.bytes[i])
